...
```bash
$ orchay orchay --web        # 웹서버 포함 실행
$ orchay orchay --web-only   # 웹서버만 실행 (스케줄링 없음)
...
### 3.7 조건부 폴링 (Tree 상태 유지)

5초 주기 폴링으로 인한 Tree 상태 초기화 문제를 해결합니다.

#### 3.7.1 문제

- 기존: 5초마다 전체 Tree HTML 교체 → WP 펼침/Task 선택 상태 초기화
- 원인: 서버가 클라이언트 UI 상태를 모름

#### 3.7.2 해결 방안: ETag 기반 조건부 응답

| 시나리오 | 동작 |
|----------|------|
| Task 상태 변경 없음 | 서버 304 반환 → DOM 건드리지 않음 → 상태 완벽 유지 |
| Task 상태 변경됨 | 서버 200 + 새 HTML → idiomorph로 최소 DOM 변경 |

**구현:**
- 서버: Task 상태 기반 해시로 ETag 생성
- 클라이언트: `If-None-Match` 헤더로 버전 전송
- 304 응답 시 HTMX가 swap 하지 않음

#### 3.7.3 기대 효과

| 항목 | 효과 |
|------|------|
| 상태 유지 | WP 펼침/접힘, Task 선택 상태 100% 보존 |
| 네트워크 | 변경 없으면 body 없는 304 응답 (대역폭 절약) |
| 성능 | 변경 없으면 서버 렌더링/클라이언트 DOM 조작 스킵 |
...
| 1.4 | 2025-12-28 | 3.7 조건부 폴링 (ETag 기반 Tree 상태 유지) 추가 |
