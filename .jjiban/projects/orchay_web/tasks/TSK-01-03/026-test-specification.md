...
def test_web_only_option_parsing():
    args = parse_args(["orchay", "--web-only"])
...
| 입력 | `["orchay", "--web", "--port", "3000"]` |
| 예상 결과 | `args.port == 3000` |
| 우선순위 | 중간 |

```python
def test_port_option_parsing():
    args = parse_args(["orchay", "--web", "--port", "3000"])
...
def test_web_options_mutually_exclusive():
    with pytest.raises(SystemExit):
        parse_args(["orchay", "--web", "--web-only"])
```
...
def test_invalid_port():
    with pytest.raises(SystemExit):
        parse_args(["orchay", "--web", "--port", "99999"])
```
...
| 절차 | 1. `orchay orchay --web` 실행<br>2. 브라우저에서 localhost:8080 접속<br>3. WBS 트리 표시 확인<br>4. Ctrl+C로 종료 |
| 예상 결과 | 모든 단계 정상 동작 |
| 우선순위 | 높음 |
