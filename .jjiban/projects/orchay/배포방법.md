...
orchay 실행 파일과 함께 hook, .wezterm.lua, .jjiban 설정 파일들을 배포하는 방법입니다.

### 8.1 배포 대상 설정 파일

| 파일 | 용도 | 필수 |
|------|------|------|
| `.jjiban/settings/columns.json` | 칸반 컬럼 정의 | 필수 |
| `.jjiban/settings/workflows.json` | 워크플로우 상태 머신 | 필수 |
...
├── columns.json          # .jjiban/settings/columns.json 복사
├── workflows.json        # .jjiban/settings/workflows.json 복사
├── wezterm.lua           # .jjiban/docs/.wezterm.lua 복사
...
def init(
    ...
    """프로젝트 초기화 - .jjiban 구조 및 설정 파일 생성"""

    base_dir = Path.cwd()

    # 1. .jjiban 디렉토리 구조 생성
    jjiban_dir = base_dir / ".jjiban"
    settings_dir = jjiban_dir / "settings"
    projects_dir = jjiban_dir / "projects" / project
...
class ConfigManager:
    ...
    def __init__(self, base_dir: Path):
        ...
        self.jjiban_dir = base_dir / ".jjiban"
        self.settings_dir = self.jjiban_dir / "settings"
...
# 4. 생성된 파일 확인
# .jjiban/settings/columns.json
# .jjiban/settings/workflows.json
# .jjiban/projects/my-project/wbs.md
# ~/.wezterm.lua
