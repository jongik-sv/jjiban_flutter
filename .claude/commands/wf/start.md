...
> **상태 전환**: `[ ] Todo` → `[an]` | `[dd]`
> **적용 category**: `defect`, `infrastructure`
> **계층 입력**: WP/ACT/Task 단위 (WP/ACT 입력 시 하위 Task 병렬 처리)
>
> ⚠️ **development 카테고리는 `/wf:design` 명령어를 사용하세요.**
...
> **참고**: development 카테고리는 `/wf:design` 명령어를 사용하세요.
...
| category | 다음 | 설명 |
|----------|------|------|
| defect | `/wf:fix` | 수정 단계 (`[an]` → `[fx]`) |
| infrastructure | `/wf:skip` 또는 `/wf:build` | 설계 생략 또는 구현 |

> **development 카테고리**: `/wf:design` → `/wf:approve` → `/wf:build`
